...
### Change Log
- **2026-03-05**: Updated lease-details lease-only expectation UX and amenity extraction targeting: `audit_engine/entrata_lease_terms.py` now prioritizes Exclusive Bedspace Addendum context for `AMENITY_PREMIUM` extraction and excludes Floor Plan Rate Addendum context from amenity candidate selection; lease-only bullet messages now render as `AR Name (Code) $Amount / frequency` (e.g., `one time`, `monthly`) without the prior `Lease expects ...` prefix/suffix; `templates/lease.html` warning box header now reads `Expected in Lease, no scheduled charges or transactions matched:`
