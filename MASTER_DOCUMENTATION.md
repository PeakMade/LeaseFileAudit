...
### Change Log
- **2026-07-20 (v3)**: Reverted pre-filter optimization from v2 — removed the block in `web/views.py` `execute_audit_run()` that filtered `scheduled_normalized` to `allowed_ar_codes` before expansion. Although the filter did not affect snapshot output (the whitelist still applies at snapshot-write time), it broke reconciliation accuracy: removing non-allowed scheduled charges before reconciliation eliminated the secondary/tertiary matching fallback that relies on `(LEASE_INTERVAL_ID, AR_CODE_ID, AUDIT_MONTH)` cross-code context; Clemson Edge regressed from 99.9% match rate / 5 exceptions to 37% match rate / 685 false `SCHEDULED_NOT_BILLED` discrepancies because rent AR transactions lost the surrounding code context that anchored interval-level matching. The `allowed_ar_codes` whitelist remains correctly applied at snapshot-write time only.
- **2026-07-20 (v2)**: Two performance optimizations to offset data-volume increase from the open-ended charge window fix: (1) **Batch concurrency raised** in `storage/service.py` — default increased from 8 to 16 concurrent Graph API $batch requests, hard cap raised from 10 to 20 (overridable via `SHAREPOINT_BATCH_CONCURRENCY_SNAPSHOTS`); reduces async snapshot write time from ~2 min to ~50s for a typical 1800-row write (91 batches / 16 = 6 rounds vs 12 previously); (2) **Pre-filter scheduled charges to `allowed_ar_codes` before expansion** in `web/views.py` `execute_audit_run()` (new `[AR CODE PRE-FILTER]` log line after Phase 3 normalization, before Phase 4 expansion) — when `allowed_ar_codes` is non-empty, scheduled charges for other AR codes are dropped before `expand_scheduled_to_months()`; without this, the open-ended charge fix would pass ~15K schedules into expansion creating ~180K monthly buckets (~90% discarded at snapshot stage); with pre-filter only ~1-3K schedules expand to ~12-36K buckets; display output is unchanged since non-allowed codes never reach snapshots.
- **2026-07-20**: Fixed two bugs that caused incorrect data to display on the property view after an API audit: (1) **Redirect bug** — `templates/audit_status.html` was building the post-audit redirect URL as `/property/{id}?run_id={run_id}` (query string), but `property_view()` in `web/views.py` only reads `run_id` from the URL path (`/property/<property_id>/<run_id>`); the query string parameter was silently ignored, `run_id` resolved to `None`, and the view fell back to `get_available_runs()` which returned whichever portfolio snapshot was most recently written to SharePoint (often a different property's run, since the new run's async snapshot write hadn't completed yet); fixed by changing the redirect to `/property/${propertyId}/${job.run_id}` (path segment). (2) **Open-ended scheduled charge filter bug** — the early audit-window pre-filter in `web/views.py` (`execute_audit_run`) replaced null `CHARGE_END_DATE` values with `CHARGE_START_DATE` via `.where(notna(), sched_start)` before computing the overlap mask; for ongoing/recurring leases that started before the audit window and have no explicit end date, this caused the effective end date to equal the pre-window start date, making the overlap condition `sched_end >= window_start` fail and dropping the charge entirely; at properties with many established leases (e.g. Cobalt Row: 14,668 of 15,247 charges dropped) this produced thousands of false `SCHEDULED_NOT_BILLED` flags and inflated phantom undercharge totals; fixed by replacing null end dates with `pd.Timestamp('2099-12-31')` so open-ended charges are always treated as ongoing within the window.
- **2026-07-17 (v2.1)**: Completed debug logging cleanup by removing remaining `[SNAPSHOT_METRICS_DEBUG]` print statements (8 locations in `storage/service.py` lines 557-572) that were missed in initial v2 pass; removed variance calculation diagnostics, exception row logging, and sample output from snapshot metrics computation in `_calculate_static_snapshot_metrics()`
- **2026-07-17 (v2)**: Implemented additional major performance optimizations reducing audit time from ~110-130s to ~60-80s (additional 40-50% reduction, total 3.5-4x faster than original baseline): (4) Parallel API Calls: modified `fetch_property_api_sources()` in `audit_engine/api_ingest.py` to fetch lease details and AR transactions simultaneously using Python threading instead of sequentially (~45 second savings); (5) Early AR Code Filtering: moved excluded AR code filtering from Phase 5 (reconciliation) to Phase 1 (immediately after API fetch), reducing data volume through all subsequent processing phases (~15-20 second savings); (6) Categorical Data Types: converted high-cardinality ID columns (`PROPERTY_ID`, `AR_CODE_ID`, `LEASE_INTERVAL_ID`) to pandas categorical dtype after normalization, reducing memory usage by 70-80% and speeding up grouping/filtering operations (~5-10 second savings); (7) Single Property Optimization: added detection for single-property audits in `web/views.py` to skip property grouping overhead when only one property is being audited (~2-3 second savings); (8) Debug Logging Removal: removed verbose debug logging from hot paths including `[BATCH_DEBUG]` (storage/service.py - 6 locations), `[RECONCILE_BUCKETS_DEBUG]` (audit_engine/reconcile.py - 32 locations), `[AR FILTER DEBUG]` (audit_engine/mappings.py - 9 locations), and column listing prints (web/views.py - 3 locations), eliminating console I/O bottleneck (~20-30 second savings); combined with previous optimizations (per_page=500, audit window expansion, batch_concurrency=4), total performance improvement: **~230s → ~60-80s (65-75% reduction, 2.9-3.8x faster)**, providing 150-170 second safety margin under Azure's 230-second timeout
- **2026-07-17 (v1)**: Implemented major performance optimizations reducing audit time from ~230s to ~110-130s (2-3x faster, 50-60% reduction): (1) API Fetch Optimization: changed `per_page` from 100 to 500 leases per request and removed `max_pages` limit in `audit_engine/api_ingest.py`, reducing API calls from 27 to 6 pages (~60 second savings); (2) Expansion Optimization: modified `expand_scheduled_to_months()` in `audit_engine/expand.py` to accept `audit_window_start` and `audit_window_end` parameters and skip months outside audit window during expansion, reducing row generation by ~60% (~20 second savings); updated `execute_audit_run()` in `web/views.py` to pass audit window parameters to expansion function; (3) SharePoint Write Optimization: increased default `batch_concurrency` from 1 to 4 in `storage/service.py` to process 4 batches in parallel instead of sequentially (~30 second savings); added new environment variables `SHAREPOINT_BATCH_CONCURRENCY_SNAPSHOTS` (default 4) and `SHAREPOINT_BATCH_CONCURRENCY_AUDITRUNS` (default 4) for Azure configuration; created `.env.performance` template and `AZURE_ENV_VARIABLES.md` documentation with complete environment variable reference; performance impact: API Fetch ~190s→~90s (-100s), Expansion ~40s→~20s (-20s), SharePoint ~40s→~10s (-30s), bringing full property audits well under 230-second Azure timeout
...
- **2026-04-01**: Fixed API scheduled-charge recurrence handling in `audit_engine/api_ingest.py` so one-time Entrata charges (for example `Move-In` and `Renewal Start`) no longer inherit explicit lease-length end dates when present in the payload; one-time charges now stay scoped to a single audit month instead of expanding across the full lease term and creating false `scheduled_not_billed` discrepancies
